...
addpath('utils')
load('models\DBSR_blur1.mat') % change it for any model as the blurring level

% set parameters
up_scale = 3;

%% change the number according to sigma (1,2,3,4) of non-blind blurring in training.
 j=1; % values 0.2 for LR, 1, 2, 3, 4 (according to blurring level)
...
im_h_y = DBSR_Concat(im_l, model, up_scale);
